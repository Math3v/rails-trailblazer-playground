...
describe 'Comment::Create' do
  ...
  it 'creates valid Comment' do
    ...
    result = Comment::Create.call(comment: { body: 'Testing body', thing: thing})
  ...
  it 'cannot create Comment for invalid Thing' do 
    result = Comment::Create.call(comment: {body: 'Testing body', thing: {}})
    expect(result.success?).to be(false)
    expect(result['contract.default'].errors.messages.inspect).to eq('{:thing=>["must be filled", "Thing does not exist"]}')
  end
